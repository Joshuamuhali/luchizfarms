...
import { Camera, Sprout, Sun, Wind, Droplets, Wheat } from "lucide-react";
...
const FarmGallerySection = () => {
  ...
  useEffect(() => {
    const fromProducts: GalleryImage[] = products
      .filter((p) => p.image_url)
      .slice(0, 9)
      .map((p) => ({
        id: p.id,
        url: normalizeSupabaseStorageUrl(p.image_url) ?? "/placeholder.svg",
        productName: p.name,
        productId: p.id,
      }));
    setGalleryImages(fromProducts);
  }, [products]);
