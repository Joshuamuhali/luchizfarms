...
const OrderSection = () => {
  ...
  const { cart, updateQty, clearCart } = usePersistedCart(user?.id);
  ...
  const cartEntries = Object.entries(cart).filter(([, qty]) => qty > 0) as [string, number][];
  ...
  const buildOrderPayload = () => ({
    user_id: user?.id,
    customer_name: customerName.trim() || profile?.full_name || "Guest",
    customer_phone: customerPhone.trim() || profile?.phone || "N/A",
  ...
  const placeOrder = async () => {
    ...
    if (!user) {
      toast({ title: "Sign in required", description: "Create an account to place orders." });
      navigate("/login", { state: { from: "/order" } });
      return;
    }
    if (!customerName.trim() || !customerPhone.trim()) {
      toast({ title: "Details required", description: "Enter your name and phone.", variant: "destructive" });
      return;
    }

    const stockError = DataService.validateCartStock(cartEntries, getProductById);
    if (stockError) {
      toast({ title: "Stock issue", description: stockError, variant: "destructive" });
      return;
    }

    setPlacing(true);
    try {
      const order = await DataService.createOrder(buildOrderPayload());
      clearCart();
      toast({ title: "Order placed!", description: "Send it for packaging when ready." });
      navigate(`/account/orders/${order.id}?step=packaging`);
    } catch (error) {
      toast({
  ...
  const sendToWhatsApp = async () => {
    ...
    const text = [
      ...
      `Name: ${customerName || "—"}`,
      `Phone: ${customerPhone || "—"}`,
      customerName && customerPhone ? "" : "(Sign in on the website for tracked orders)",
    ]
      .filter(Boolean)
      .join("\n");
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-20 space-y-6">
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto leading-relaxed">
            Browse fresh vegetables and meat, place your order online, send it for packaging,
            then pay when ready.{" "}
            {!user && (
              <Link to="/login" className="text-farm-leaf font-semibold hover:underline">
                Sign in
              </Link>
            )}{" "}
            to track orders.
              ...
              <div className="border-t border-farm-soil/20 pt-6">
                ...
                {!user && (
                  <p className="text-sm text-center text-muted-foreground mb-4">
                    <Link to="/login" className="text-farm-leaf font-medium hover:underline">
                      Create an account
                    </Link>{" "}
                    to place orders, send for packaging, and pay online.
                  </p>
                )}
                <div className="grid md:grid-cols-3 gap-4">
                  <button
                    onClick={placeOrder}
                    disabled={placing}
                    className="btn-farm group flex items-center justify-center gap-3 text-lg md:col-span-2"
                  >
                ...
                <button
                  ...
                  type="button"
                  className="w-full mt-3 flex items-center justify-center gap-2 text-sm text-muted-foreground hover:text-farm-leaf"
                >
